...
@requires_display
def test_a_jump_out_of_a_long_month_lands_on_the_next_month(seeded_window, pump) -> None:
    """The case that kept coming back: the next month is thousands of photos
    away, so the jump has to pull pages in before it can go anywhere, and
    every attempt to measure across that load put the view somewhere in the
    middle of the collection with no header in reach.

    It now loads behind a spinner and places itself from the target row's own
    geometry once the list has caught up. Measured here on a 2400-photo
    month: the write lands on the header's exact content Y."""
    _seed_months(seeded_window, [(0, 40), (1, 2400), (2, 60)])
    seeded_window.settings.sort_modes = {"photos": "date"}
    seeded_window._render()
    _realised(seeded_window, pump)

    grid = seeded_window.gallery_grid
    store = grid.row_store
    ...
    source_pos = headers[-1]                      # the long month itself
    assert grid._find_adjacent_header_pos(source_pos, +1) is None, (
        "the next month is already loaded — this is not the deferred case"
    )

    targets = []
    real_scroll_to = grid.grid_view.scroll_to
    grid.grid_view.scroll_to = lambda pos, flags, s: (
        targets.append((pos, store.get_item(pos))), real_scroll_to(pos, flags, s))[1]
    writes = []
    real_write = grid._set_vadj_clamped
    grid._set_vadj_clamped = lambda v, val: (writes.append(val), real_write(v, val))[1]

    source = SimpleNamespace(get_position=lambda: source_pos, get_child=lambda: None)
    grid._begin_deferred_jump(source, 0.0)
    assert grid._jump_state is not None, "it did not defer"
    assert grid.pull_revealer.get_reveal_child() is True, "no spinner while loading"

    _settle(pump, rounds=150)

    assert grid._jump_state is None, "the jump never finished"
    assert grid.pull_revealer.get_reveal_child() is False, "the spinner stayed up"
    assert seeded_window._suppress_front_eviction is False, "eviction still held"
    assert targets, "it never scrolled anywhere"
    target_pos, target_row = targets[-1]
    assert target_row.is_header, "it aimed at something that is not a month"
    assert target_pos > source_pos

    # Every row in this view is the same height, so the header's content Y is
    # exactly that many rows down — and the jump asked for precisely that.
    row_h = grid._measure_row_heights(
        next(li.get_child() for li in grid._bound_list_items
             if li.get_item() is not None and li.get_item().is_header)
    )[1]
    assert writes[-1] == pytest.approx(target_pos * row_h, abs=row_h / 2), (
        f"asked for {writes[-1]}, header sits at {target_pos * row_h}"
    )
