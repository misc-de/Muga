...
Yaga uses standard gettext catalogues (``po/*.po``), so translators can work
...
1. ``yaga/data/locale/<lang>/LC_MESSAGES/yaga.mo`` — what an installed copy
   (pip, Flatpak, ``install.sh``) ships.
2. ``po/<lang>.po`` — parsed directly, so a plain ``python3 -m yaga`` from a
...
class _PoCatalogue(gettext.NullTranslations):
    ...
    Only what Yaga uses: singular messages, no plural forms, no contexts.
    """
...
def system_language() -> str:
    """The system's language if Yaga has a catalogue for it, else English."""
    try:
