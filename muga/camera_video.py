...
class CameraVideoMixin:
    ...
    def _start_video_recording(self) -> None:
        ...
        _dlog(f"[muga.camera] video-record: stopping preview for {device.get('name', cam_id)}")
        self._stop_pipeline()
    ...
    def _video_pipeline_build_generic(self, device: dict[str, Any]) -> bool:
        ...
        pipeline = gst.Pipeline.new("muga-video-record-generic")
        ...
        _dlog(f"[muga.camera] video-record: generic MJPEG-in-Matroska, "
    ...
    def _video_pipeline_build(self, cam_id: int) -> bool:
        ...
        pipeline = gst.Pipeline.new("muga-video-record")
        ...
        jpegenc.link(mkvmux)
        mkvmux.link(filesink)
        _dlog(f"[muga.camera] video-record: MJPEG-in-Matroska, jpeg quality={jpeg_q}")
        ...
        result = pipeline.set_state(gst.State.PLAYING)
        _dlog(f"[muga.camera] video-record: pipeline PLAYING -> "
    ...
    def _stop_video_recording(self) -> None:
        ...
        if self._video_finalize_source is not None:
            ...
            return
        _dlog("[muga.camera] video-record: stop requested")
        self._shutter.set_sensitive(False)
        self._show_capture_spinner(True)
    ...
    def _on_video_pipeline_eos(self, _bus: Any, _msg: Any) -> None:
        _dlog("[muga.camera] video-record: EOS received, finalising file")
    ...
    def _video_finalize_timeout(self) -> bool:
        ...
        if self._video_pipeline is not None:
            _dlog("[muga.camera] video-record: EOS timeout, finalising anyway")
            self._video_finalize()
        return False
    ...
    def _video_recording_failed(self, reason: str) -> bool:
        _dlog(f"[muga.camera] video-record: {reason}")
        self._video_teardown()
        self._recording = False
        self._apply_flash_to_pipeline()
        self._stop_record_blink()
    ...
    def _on_video_pipeline_error(self, _bus: Any, msg: Any) -> None:
        try:
            ...
            _dlog(f"[muga.camera] video-record bus error: "
