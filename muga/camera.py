...
# Set MUGA_CAMERA_DEBUG=1 in the environment to surface the camera's
# pipeline diagnostics on stderr — see camera_debug.py.

from .camera_capture_io import CameraCaptureIOMixin
from .camera_debug import dlog as _dlog
from .camera_menus import CameraMenusMixin
from .camera_video import CameraVideoMixin
...
_OPTIONS_NOTCH_MARGIN = 40   # gap above the options bar (clear of notch
                             # in portrait).
_OPTIONS_BAR_SIDE = 30       # options-bar edge inset in landscape — per
                             # user spec ("icon im landscape 30px vom
                             # rahmen weg").
_SHUTTER_SIDE_MARGIN = 24    # shutter inset from screen edge in portrait
                             # handed mode (corner placement).
_SHUTTER_LANDSCAPE_INSET = 70  # shutter edge inset in landscape — per user
                               # spec ("auslöser 70px im normal modus").
...
class CameraWindow(
    ...
    def __init__(
        ...
        # Settings object (muga.config.Settings) for persisting camera
        ...
        self._orientation: OrientationClient | None = OrientationClient()
    ...
    def _start_pipeline(self) -> bool:
        ...
        pipeline = gst.Pipeline.new("muga-camera")
        pipeline.add(source)
        ...
        if _is_halium_device(device):
            try:
                ...
                templates = []
            _dlog(f"[muga.camera] droidcamsrc pad templates: {templates}")
        ...
        _dlog(f"[muga.camera] pipeline PLAYING source={src_factory} "
    ...
    def _on_source_buffer(self, _pad: Any, _info: Any) -> Any:
        ...
        if self._source_frame_count <= 3:
            _dlog(f"[muga.camera] source buffer #{self._source_frame_count}")
        if self._source_frame_count >= 3:
    ...
    def _on_sink_buffer(self, _pad: Any, _info: Any) -> Any:
        ...
        if self._sink_frame_count <= 3:
            _dlog(f"[muga.camera] sink buffer #{self._sink_frame_count} "
    ...
    def _on_preview_sample(self, sink: Any) -> Any:
        ...
        if self._preview_frame_count == 0:
            _dlog(f"[muga.camera] first preview frame {w}x{h} "
    ...
    def _on_bus_message(self, _bus: Any, message: Any) -> None:
        ...
        if t == gst.MessageType.ERROR:
            ...
            _dlog(f"[muga.camera] bus WARNING from {message.src.get_name() if message.src else '?'}: "
                f"{err.message if err else '?'} | {(dbg or '').strip()}")
        elif t == gst.MessageType.EOS:
            _dlog(f"[muga.camera] bus EOS from {message.src.get_name() if message.src else '?'}")
        elif t == gst.MessageType.STATE_CHANGED:
            if message.src is self._pipeline:
                old, new, pending = message.parse_state_changed()
                if new == gst.State.PLAYING or pending != gst.State.VOID_PENDING:
                    _dlog(f"[muga.camera] pipeline state {old.value_nick} -> "
    ...
    def _populate_resolutions(self, device: dict[str, Any]) -> None:
        ...
        for w, h in resolutions:
            ...
            row._muga_res = (w, h)  # type: ignore[attr-defined]
            list_box.append(row)

        def on_activated(_lb: Gtk.ListBox, row: Gtk.ListBoxRow) -> None:
            wh = getattr(row, "_muga_res", None)
            popover.popdown()
    ...
    def _try_start_geo_silent(self) -> None:
        ...
        if self._geo is not None:
            return
        self._geo = GeoClient(app_id="muga")
        ok = self._geo.start(
            accuracy=8,
    ...
    def _capture(self) -> None:
        ...
        if sink is None:
            _dlog(f"[muga.camera] capture: ignored (imgsink={self._imgsink is not None}, "
                f"appsink={self._appsink is not None})")
            return
        path_name = "imgsrc" if sink is self._imgsink else "vfsrc+jpegenc"
        _dlog(f"[muga.camera] capture: start path={path_name}")
        ...
        if (sink is self._appsink
            ...
            try:
                ...
                _dlog("[muga.camera] capture: caps swapped to high-res "
                    "(waiting for renegotiated frame)")
            except Exception as exc:
                self._capture_saved_caps = None
                self._capture_min_width = 0
                _dlog(f"[muga.camera] capture: caps swap failed: {exc}")
        ...
        if sink is self._imgsink:
            ...
            self._emit_start_capture_async(
                ...
                delay_ms=150,
                log_prefix="capture",
                thread_name="muga-start-capture",
            ...
            if self._valve is not None:
                ...
                _dlog("[muga.camera] capture: valve opened")
    ...
    def _close_valve_and_disconnect(self) -> None:
        ...
        if self._capture_saved_caps is not None and self._capsfilter is not None:
            try:
                ...
                _dlog("[muga.camera] capture: restored 720p preview cap")
            except Exception:
                LOGGER.debug("camera cleanup/op failed", exc_info=True)
        self._capture_saved_caps = None
        self._capture_min_width = 0
    ...
    def _on_capture_sample(self, sink: Any) -> Any:
        ...
        try:
            ...
            sample = None
            _dlog(f"[muga.camera] capture: pull-sample failed: {exc}")
        if sample is None:
            return gst.FlowReturn.OK
        ...
        if self._capture_min_width > 0:
            ...
            if ok_w and w < self._capture_min_width:
                _dlog(f"[muga.camera] capture: skipping low-res frame "
                    f"({w}px, waiting for >={self._capture_min_width}px)")
                return gst.FlowReturn.OK
            # First matching frame — stop filtering.
            self._capture_min_width = 0
            _dlog(f"[muga.camera] capture: high-res frame received ({w}px)")
        ...
        if sig_sink is not None and sig_id is not None:
            try:
                ...
                LOGGER.debug("camera cleanup/op failed", exc_info=True)
        _dlog("[muga.camera] capture: new-sample accepted")
    ...
    def _finish_capture(self, sample: Any) -> bool:
        ...
        if self._closing:
            return False
        if sample is None:
            _dlog("[muga.camera] capture: finish with no sample")
    ...
    def _on_capture_timeout(self) -> bool:
        ...
        _dlog("[muga.camera] capture: TIMEOUT — no sample arrived in 10 s")
        self._capture_timeout_id = None
        self._close_valve_and_disconnect()
        self._show_toast(self._("No frame available"))
        self._busy_capture = False
    ...
    def _capture_via_image_pipeline(self) -> None:
        ...
        _dlog(f"[muga.camera] image-capture: stopping preview for cam {cam_id}")
        self._stop_pipeline()
    ...
    def _image_pipeline_build(self, cam_id: int) -> bool:
        ...
        pipeline = gst.Pipeline.new("muga-image-capture")
        src = gst.ElementFactory.make("droidcamsrc", "src")
        if src is None:
    ...
    def _on_image_capture_sample(self, sink: Any) -> Any:
        ...
        try:
            ...
            _dlog(f"[muga.camera] image-capture: pull-sample failed: {exc}")
            sample = None
        if sample is None:
            return gst.FlowReturn.OK
        ...
        if s is not None:
            ok_w, w = s.get_int("width")
            ok_h, h = s.get_int("height")
            _dlog(f"[muga.camera] image-capture: sample received {w}x{h}")
    ...
    def _on_image_capture_timeout(self) -> bool:
        _dlog("[muga.camera] image-capture: TIMEOUT — no sample arrived in 15 s")
        self._image_timeout_id = None
        self._image_teardown()
        self._show_toast(self._("No frame available"))
        self._show_capture_spinner(False)
    ...
    def _image_capture_failed(self, reason: str) -> bool:
        _dlog(f"[muga.camera] image-capture: {reason}")
        self._image_teardown()
        if self._closing:
            self._busy_capture = False
            return False
    ...
    def _on_image_pipeline_error(self, _bus: Any, msg: Any) -> None:
        ...
        try:
            ...
            _dlog(f"[muga.camera] image-capture bus error: "
                f"{err.message if err else '?'} | {(dbg or '').strip()}")
