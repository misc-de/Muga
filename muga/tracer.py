...
inside the ``muga`` package is logged (function name, file:line, repr'd args,
...
def _is_muga_frame(frame) -> bool:
    fname = frame.f_code.co_filename
    return f"{os.sep}muga{os.sep}" in fname


def _profile(frame, event, _arg):
    if event != "call":
        return
    # Track liveness on EVERY Python call (not just muga frames) so the watchdog
    # doesn't fire false alarms while main is bouncing through gi/Adw callbacks.
    thread = threading.current_thread()
    tid = thread.ident or 0
    _last_event_time[tid] = time.monotonic()

    if not _is_muga_frame(frame):
        return
    code = frame.f_code
...
def install(path: Path | None = None) -> Path:
    ...
    _trace_file.write(
        f"=== Muga trace started {time.time():.4f} pid={os.getpid()} ===\n"
    )
    _trace_file.flush()
    sys.setprofile(_profile)
    threading.setprofile(_profile)
    threading.Thread(
        target=_watchdog, name="muga-trace-watchdog", daemon=True
    ).start()
    print(f"[muga] tracing enabled -> {target}", file=sys.stderr)
    return target
