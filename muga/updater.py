"""In-app update checker and installer for Muga (git pull or zip download).

Mirrors the DrivePulse updater: when Muga runs from a git checkout (the
default — install.sh keeps the source in place and launches it with
``python -m muga``), an update is a ``git pull``. When it runs from a plain
...
# Repo root: muga/updater.py → muga/ → <repo>. This is where .git lives and
# where the zip overlay is applied.
_APP_DIR = Path(__file__).resolve().parent.parent
_GITHUB_REPO = "misc-de/Muga"
_DEFAULT_BRANCH = "main"
# The canonical version string lives in muga/__init__.py; we read it from the
# remote (raw / git show) and parse VERSION out of it.
_RAW_INIT_URL = f"https://raw.githubusercontent.com/{_GITHUB_REPO}/{{branch}}/muga/__init__.py"
_ZIP_URL = f"https://github.com/{_GITHUB_REPO}/archive/refs/heads/{{branch}}.zip"
_USER_AGENT = f"Muga/{VERSION}"
_VERSION_RE = re.compile(r"""VERSION\s*=\s*['"]([^'"]+)['"]""")

def is_flatpak() -> bool:
    """True when Muga runs inside a Flatpak sandbox.
...
# near this is a decompression bomb or the wrong URL, not a Muga release.
...
# ---------------------------------------------------------------------------
# HTTP helpers (stdlib only — Muga has no requests dependency)
# ---------------------------------------------------------------------------
...
def _check_git() -> UpdateInfo:
    ...
    if not behind:
        return UpdateInfo(False, None)
    _, init_text = _git("show", f"origin/{branch}:muga/__init__.py")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            with zipfile.ZipFile(zip_path) as zf:
                ...
                # user's disk. (Muga's own tree is a couple of MB.)
        ...
        # GitHub extracts to a single subdirectory (e.g. Muga-main).
