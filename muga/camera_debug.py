...
LOGGER = logging.getLogger("muga.camera")
...
CAMERA_DEBUG = bool(os.environ.get("MUGA_CAMERA_DEBUG"))
...
def dlog(message: str) -> None:
    ...
    additionally mirrors to stderr when MUGA_CAMERA_DEBUG is set — useful when
    running Muga from a phone terminal where logger output may not be visible.
    """
    LOGGER.info(message)
    if CAMERA_DEBUG:
        print(message)
